...
const VERDICT_CONFIG = {
  'FAKE':         { color: 'var(--red)',   bg: 'var(--red-dim)',    border: 'rgba(255,59,92,0.3)',   icon: '✕', label: 'FAKE NEWS' },
  'LIKELY FAKE':  { color: '#ff7a42',      bg: '#1f0e04',           border: 'rgba(255,122,66,0.3)',  icon: '⚠', label: 'LIKELY FAKE' },
  'UNCERTAIN':    { color: 'var(--yellow)',bg: 'var(--yellow-dim)', border: 'rgba(255,204,0,0.3)',   icon: '?', label: 'UNCERTAIN' },
  'LIKELY REAL':  { color: '#44ddaa',      bg: '#001810',           border: 'rgba(68,221,170,0.3)',  icon: '~', label: 'LIKELY REAL' },
  'REAL':         { color: 'var(--green)', bg: 'var(--green-dim)',  border: 'rgba(0,229,160,0.3)',   icon: '✓', label: 'VERIFIED REAL' },
};
...
// Simple markdown renderer for bold + newlines
...
export default function VerdictCard({ result }) {
  ...
  return (
    <div style={{
      ...
      padding: '28px',
      boxShadow: `0 0 60px rgba(0,0,0,0.4), inset 0 1px 0 ${cfg.border}`,
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          <div style={{
            ...
            boxShadow: `0 0 20px ${cfg.color}33`, position: 'relative',
          }}>
            {cfg.icon}
            <div style={{
              position: 'absolute', inset: '-4px', borderRadius: '50%',
              border: `1px solid ${cfg.color}`, animation: 'pulse-ring 2.5s ease-in-out infinite',
            }} />
          </div>
          <div>
      ...
      <div style={{ minHeight: '120px' }}>
          ...
          <div>
            ...
            {result.citedSources?.length > 0 && (
              <div style={{
                ...
                <div style={{ fontFamily: 'var(--font-mono)', color: 'var(--text-muted)', fontSize: '10px', letterSpacing: '0.08em', marginBottom: '8px' }}>
                  CITED SOURCES
                </div>
                {result.citedSources.map((cs, i) => (
                  <div key={i} style={{ display: 'flex', gap: '8px', marginBottom: '4px', alignItems: 'flex-start' }}>
                    <span style={{
                      flexShrink: 0, width: '22px', height: '16px',
                    ...
                    <a href={cs.url} target="_blank" rel="noopener noreferrer" style={{
                      color: 'var(--text-secondary)', textDecoration: 'none', fontSize: '12px', lineHeight: '1.4',
                    }}>
                      {cs.source || cs.title}
        ...
        {activeTab === 'reasoning' && (
          <ReasoningChain steps={result.reasoningSteps || []} />
        )}

        {activeTab === 'score' && result.scoreBreakdown && (
          <ScoreBreakdown breakdown={result.scoreBreakdown} color={cfg.color} />
        )}
      </div>
...
function SourcesList({ sources }) {
  if (!sources.length) return (
    <div style={{ fontSize: '13px', color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', textAlign: 'center', padding: '24px' }}>
      No sources found
    </div>
  );

  // Group: fact-checks first
  ...
  const ordered = [...factChecks, ...others];
      ...
      {ordered.map((src, i) => {
        ...
        return (
          <div key={i} style={{
            ...
            background: isFC ? 'rgba(170,102,255,0.05)' : 'rgba(0,0,0,0.2)',
            border: `1px solid ${isFC ? 'rgba(170,102,255,0.2)' : 'var(--border)'}`,
            borderRadius: 'var(--radius)', padding: '10px 12px',
          }}>
            <div style={{
              width: '8px', height: '8px', borderRadius: '50%', marginTop: '5px', flexShrink: 0,
              background: credColor, boxShadow: `0 0 6px ${credColor}66`,
            }} />
            <div style={{ flex: 1, minWidth: 0 }}>
              {isFC && src.factCheckRating && (
                <div style={{
                  fontSize: '10px', fontFamily: 'var(--font-mono)', color: '#aa66ff',
                  background: 'rgba(170,102,255,0.1)', border: '1px solid rgba(170,102,255,0.2)',
              ...
              <a href={src.url} target="_blank" rel="noopener noreferrer" style={{
                display: 'block', fontSize: '13px', color: 'var(--blue)',
                textDecoration: 'none', fontWeight: '500', marginBottom: '3px',
                overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
              }}>{src.title || src.source}</a>
              {src.snippet && (
                <p style={{
            ...
            <div style={{ flexShrink: 0, textAlign: 'right' }}>
              <span style={{ fontSize: '10px', fontFamily: 'var(--font-mono)', color: credColor }}>
                {src.credibility}
              </span>
              {src.publishedAt && (
...
function ReasoningChain({ steps }) {
  if (!steps.length) return (
    <p style={{ fontSize: '13px', color: 'var(--text-muted)', fontFamily: 'var(--font-mono)' }}>No reasoning steps available</p>
  );
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
      {steps.map((step, i) => (
        <div key={i} style={{ display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
          <span style={{
            ...
            padding: '2px 7px', background: 'var(--blue-dim)',
            border: '1px solid rgba(68,136,255,0.2)', borderRadius: '4px',
            flexShrink: 0, marginTop: '1px', minWidth: '28px', textAlign: 'center',
          }}>{String(i + 1).padStart(2, '0')}</span>
          <span style={{ fontSize: '13px', color: 'var(--text-secondary)', fontFamily: 'var(--font-mono)', lineHeight: '1.5' }}>
            {step}
          </span>
        </div>
      ))}
  ...
  return (
    <div>
      <div style={{ marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '12px' }}>
        ...
        <span style={{ fontSize: '22px', fontFamily: 'var(--font-display)', fontWeight: '800', color }}>
          {breakdown.score > 0 ? '+' : ''}{breakdown.score?.toFixed(1)}
        </span>
      </div>
        ...
        return (
          <div key={key} style={{ marginBottom: '10px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '3px' }}>
              ...
              <span style={{ fontSize: '11px', fontFamily: 'var(--font-mono)', color: barColor }}>
                {val > 0 ? '+' : ''}{val.toFixed(1)}
              </span>
            </div>
