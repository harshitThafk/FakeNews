...
# Load from ml_service/.env first, then fall back to backend/.env
_here = os.path.dirname(os.path.abspath(__file__))
load_dotenv(os.path.join(_here, ".env"))                          # ml_service/.env
load_dotenv(os.path.join(_here, "..", "backend", ".env"))         # backend/.env (fallback)

# Log which keys were found so we can confirm at startup
logging.basicConfig(level=logging.INFO)
_startup_logger = logging.getLogger("startup")
_startup_logger.info(f"OPENAI_API_KEY  : {'SET ✓' if os.getenv('OPENAI_API_KEY') else 'MISSING ✗'}")
_startup_logger.info(f"SERPAPI_KEY     : {'SET ✓' if os.getenv('SERPAPI_KEY') else 'MISSING ✗'}")
_startup_logger.info(f"NEWSAPI_KEY     : {'SET ✓' if os.getenv('NEWSAPI_KEY') else 'MISSING ✗'}")
...
app = FastAPI(title="Fake News Checker ML Service", version="1.0.0", lifespan=lifespan)
