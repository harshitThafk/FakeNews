...
export default function HomePage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Fetch final
      const { data } = await fetch(`${process.env.REACT_APP_API_URL || '/api'}/check/${id}`)
        .then(r => r.json())
        .then(d => ({ data: d }));

      clearInterval(stageTimer);
      setResult(data || (await fetch(`/api/check/${id}`).then(r => r.json())));
      setActiveStage(null);
      setLoading(false);
    } catch (err) {
  ...
  const handleInputChange = (e) => {
    ...
    setInputValue(val);
    if (isUrl(val.trim())) setInputMode('url');
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '48px 24px' }}>
      {/* Hero */}
      <div style={{ textAlign: 'center', marginBottom: '48px', animation: 'fade-up 0.5s ease forwards' }}>
        <div style={{
        ...
        <p style={{
          ...
          Paste article text or a URL. Our pipeline runs ML classification,
          live search corroboration, RAG retrieval, and agentic reasoning.
        </p>
      </div>

      {/* Input form */}
      <form onSubmit={handleSubmit} style={{ marginBottom: '32px', animation: 'fade-up 0.5s ease 0.1s both forwards', opacity: 0 }}>
        {/* Mode toggle */}
          ...
          {['text', 'url'].map(mode => (
            <button
              ...
              style={{
                ...
                background: inputMode === mode ? 'var(--bg-elevated)' : 'transparent',
                color: inputMode === mode ? 'var(--text-primary)' : 'var(--text-muted)',
                outline: inputMode === mode ? '1px solid var(--border-bright)' : '1px solid transparent',
                transition: 'all var(--transition)',
              }}
            >{mode === 'text' ? '📄 TEXT' : '🔗 URL'}</button>
          ))}
        </div>
        ...
        <button
          ...
          style={{
            ...
            background: loading || !inputValue.trim()
              ? 'var(--bg-elevated)'
              : 'linear-gradient(135deg, var(--blue), #6644ff)',
            color: loading || !inputValue.trim() ? 'var(--text-muted)' : 'white',
            fontSize: '14px', fontFamily: 'var(--font-display)',
            fontWeight: '700', letterSpacing: '0.02em',
            transition: 'all var(--transition)',
            boxShadow: loading || !inputValue.trim() ? 'none' : '0 4px 20px rgba(68,136,255,0.3)',
          }}
        >
      ...
      {/* Example claims */}
      {!loading && !result && (
        <div style={{ animation: 'fade-up 0.5s ease 0.2s both forwards', opacity: 0 }}>
          <div style={{
            ...
            {EXAMPLES.map((ex, i) => (
              ...
              >
                <span style={{
                  ...
                  color: ex.type === 'fake' ? 'var(--red)' : 'var(--green)',
                  marginRight: '10px', letterSpacing: '0.06em',
      ...
      {/* Pipeline progress */}
      {loading && activeStage && (
        <PipelineProgress activeStage={activeStage} completed={false} />
      )}
